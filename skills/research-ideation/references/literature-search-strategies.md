...
**集成工具**：
- **Obsidian**（主要工具，文件系统优先）
  - 在 `Research/{project-slug}/Papers/` 中创建论文笔记
  - 自动生成 citekey 作为文件名
  - 通过 Wikilink 连接相关论文和知识
  - 使用 Dataview 查询和管理文献
  - 支持 PDF 附件和标注
...
从 local-websearch (quick_search) 搜索结果中提取 DOI 的常见方式：
...
```
local-websearch (quick_search) 搜索论文
    ↓
从搜索结果中提取论文信息
    ↓
创建 Obsidian 论文笔记（包含 frontmatter）
    ↓
保存到 Research/{project-slug}/Papers/
    ↓
通过 Wikilink 连接相关论文和知识
```
...
1. 使用 local-websearch 搜索：`quick_search(query_string="transformer interpretability", sources=['arxiv', 'semantic'])`
2. 从搜索结果中提取论文信息（标题、作者、DOI、摘要等）
3. 创建 Obsidian 论文笔记，自动生成 citekey
4. 保存为 Markdown 文件到项目的 Papers 目录

### 5.3 无 DOI 论文处理

部分论文可能没有标准 DOI：
- **arXiv 预印本**：使用 arXiv ID 作为标识
- **会议论文集**：尝试从出版商页面获取 DOI
- **无法获取 DOI**：使用论文标题和 URL 作为标识，手动创建笔记
