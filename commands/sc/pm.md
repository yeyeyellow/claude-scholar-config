...
- **State Questions**: "What's the progress", "status", "how far along" trigger context report
- **Vague Requests**: "Want to build", "want to implement", "how do I" trigger discovery mode
...
4. Act (Improvement):
   - Success → docs/patterns/[pattern-name].md (formalize)
...
```
User: "Want to add authentication to app"
...
Error Detection Protocol:
  ...
  2. Root Cause Investigation (MANDATORY):
     ...
     → Document: "Error cause is believed to be [X]. Because [evidence Y]"

  3. Hypothesis Formation:
     - Create docs/pdca/[feature]/hypothesis-error-fix.md
     - State: "Cause is [X]. Basis: [Y]. Solution: [Z]"
     - Rationale: "[Why this approach will solve it]"
...
Anti-Patterns (ABSOLUTELY PROHIBITED):
  ❌ "Error occurred. Let's try again"
  ❌ "Retry: 1st attempt... 2nd attempt... 3rd attempt..."
  ❌ "Timeout so increase wait time" (ignoring root cause)
  ❌ "Warning present but it works so OK" (future technical debt)

Correct Patterns (MANDATORY):
  ✅ "Error occurred. Investigate with official documentation"
  ✅ "Cause: Environment variable not set. Why needed? Understand specification"
  ✅ "Solution: Add .env + startup validation implementation"
  ✅ "Learning: Next time run environment variable check first"
```

### Warning/Error Investigation Culture

**Rule: Investigate ALL warnings and errors with curiosity.**
...
```yaml
Structure (Clear, Easy to Understand):
  docs/pdca/[feature-name]/
    ├── plan.md           # Plan: hypothesis, design
    ├── do.md             # Do: experiments, trial-and-error
    ├── check.md          # Check: evaluation, analysis
    └── act.md            # Act: improvement, next actions

Template - plan.md:
  # Plan: [Feature Name]

  ## Hypothesis
  [What to implement, why this approach]
  ...
  ## Risks & Mitigation
  - [Risk 1] → [Countermeasure]
  - [Risk 2] → [Countermeasure]
...
After each successful implementation:
  - Create docs/patterns/[feature-name].md (formalize)
